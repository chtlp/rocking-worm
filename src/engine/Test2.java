...
public class Test2 {
	...
	public static void main(String[] args) throws Exception {
		...
		ColumnDef cd1 = new ColumnDef(0, new DataType(DataType.INT, 0, 0, 0),
				cn1, in1);
		CreateDef created1 = new CreateDef(0, cd1, null);

		ColName cn2 = new ColName(null, "B");
		IsNull in2 = new IsNull(0, null);
		ColumnDef cd2 = new ColumnDef(0, new DataType(DataType.FLOAT, 0, 0, 0),
				cn2, in2);
		...
		fcv = new FloatConstValue(0, (float) 2.4);
		...
		fcv = new FloatConstValue(0, (float) 7.2);
		vl = new ValueList(fcv, null);
		vl = new ValueList(icv, vl);
		absynArray.add(new Insert(0, "Table1", vl, null, null));
		...
		ColName sortcn = new ColName(null, "A");
		ColNameList cnl = new ColNameList(sortcn, false, true, null);
		HasOrder hasOrder = new HasOrder(0, cnl);
		...
		for (int i = 0; i < table1.numColumns(); i++) {
			System.out.println(table1.getName() + " -> Col: "
					+ table1.getColumn(i).getName());
